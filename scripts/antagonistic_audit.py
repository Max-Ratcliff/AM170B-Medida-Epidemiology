...
def analyze_bias(csv_path):
    ...
    stats = raw_df.groupby('location').agg({
        'population': 'first',
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(merged['population'], merged['improvement'], alpha=0.6, color='#2c7fb8', edgecolor='white')
    plt.xscale('log')
    plt.yscale('log')
    ...
    plt.subplot(1, 2, 2)
    plt.scatter(merged['new_cases_smoothed'], merged['improvement'], alpha=0.6, color='#e31a1c', edgecolor='white')
    plt.xscale('log')
    plt.yscale('log')
    ...
    plt.tight_layout()
    output_png = os.path.join(project_root, 'outputs/antagonistic_bias_check.png')
    plt.savefig(output_png, dpi=300)
    print(f"[*] Bias check plot saved to {output_png}")
    
    # Correlation analysis
    corr_pop = merged['improvement'].corr(merged['population'], method='spearman')
    corr_cases = merged['improvement'].corr(merged['new_cases_smoothed'], method='spearman')
    ...
    print(high_imp[['country', 'improvement', 'population', 'new_cases_smoothed']])
